...
export default function ProfileTable({ data }: { data: IVote[] }) {
  ...
  const supabse = createSupabaseBrowser();
  ...
  const toastDelete = async (id: string, title: string) => {
    const deletVote = async () => {
      ...
      if (error) {
        ...
        setVotes((currentVote) => currentVote.filter((vote) => vote.id !== id));
      }
    };
          ...
          <TableBody ref={animationParent}>
            {votes.map((vote) => (
              <TableRow key={vote.id}>
                ...
                <TableCell>{new Date(vote.end_date).toDateString()}</TableCell>
                <TableCell>
                  <Popover>
                    ...
                    <PopoverContent align="end" className="w-[200px]">
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                              onClick={() => toastDelete(vote.id, vote.title)}
                            >
                              Continue
                            </AlertDialogAction>
                          </AlertDialogFooter>
                      ...
                      <Link href={`/edit/${vote.id}?user=${vote.created_by}`}>
                        <Button
                          variant="ghost"
                          className="flex items-center justify-between cursor-pointer w-full"
                        >
